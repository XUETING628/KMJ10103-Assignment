    ...
    # Data for Machine 2
    df_machine <- X005 %>% filter(Machine == 2)

    # Ensure data is numeric and sufficient for process capability
    if (nrow(df_machine) < 2) {
        warning("Not enough data for Machine 2 to create Process Capability chart. Skipping.")
        p_pc_machine <- plot_ly() %>%
                        layout(
                          title = list(text = "Not enough data for Process Capability Chart (Machine 2)", font = list(size = 20)),
        ...
        # Process capability for PartLength for Machine 2
        ...
        p_pc_machine <- plot_ly(x = hist_data_machine$mids, y = hist_data_machine$counts, type = 'bar', name = 'Frequency') %>%
          ...
          layout(
            title = list(text = paste0('Process Capability for Part Length (Machine 2)<br>LSL: ', LSL, ', USL: ', USL, '<br>Cp: ', round(Cp_value, 2), ', Cpk: ', round(Cpk_value, 2)), font = list(size = 20)),
    ...
    # Save as HTML widget
    htmlwidgets::saveWidget(as_widget(p_pc_machine), file = "media/plots/plot/process_capability_machine_2.html", selfcontained = TRUE)
