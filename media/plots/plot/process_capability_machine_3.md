    ...
    if (nrow(df_machine) < 2) {
        warning("Not enough data for Machine 3 to create Process Capability chart. Skipping.")
        p_pc_machine <- plot_ly() %>%
                        layout(
                          title = list(text = "Not enough data for Process Capability Chart (Machine 3)", font = list(size = 20)),
        ...
        # Process capability for PartLength for Machine 3
    ...
    # Save as HTML widget
    htmlwidgets::saveWidget(as_widget(p_pc_machine), file = "media/plots/plot/process_capability_machine_3.html", selfcontained = TRUE)
